...
CSHA1::CSHA1()
{
	_block = (SHA1_WORKSPACE_BLOCK *)_workspace;
...
void CSHA1::Update(UINT_8 *data, UINT_32 len)
{
	...
	if((j + len) > 63)
	{
		...
		for(; i + 63 < len; i += 64) Transform(_state, &data[i]);
...
void CSHA1::Final()
{
	...
	for(i = 0; i < 8; i++)
		finalcount[i] = (UINT_8)((_count[((i >= 4) ? 0 : 1)]
	...
	for(i = 0; i < 20; i++)
	{
		_digest[i] = (UINT_8)((_state[i >> 2] >> ((3 - (i & 3)) * 8) ) & 255);
	}
...
void CSHA1::ReportHash(char *szReport, unsigned char uReportType)
{
	...
	{
		sprintf(szTemp, "%02X", _digest[0]);
		strcat(szReport, szTemp);

		for(i = 1; i < 20; i++)
		{
			sprintf(szTemp, " %02X", _digest[i]);
			strcat(szReport, szTemp);
		}
	}
	else if(uReportType == REPORT_DIGIT)
	{
		sprintf(szTemp, "%u", _digest[0]);
		strcat(szReport, szTemp);

		for(i = 1; i < 20; i++)
		{
			sprintf(szTemp, " %u", _digest[i]);
			strcat(szReport, szTemp);
		}
	}
