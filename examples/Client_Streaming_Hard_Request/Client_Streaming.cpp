...
int main( int argc, char** argv ) {
	...
	ts->get_oauth().set_consumer_key( "MQvDXsNBq6desAPZIy9gpQ" );
	ts->get_oauth().set_consumer_secret( "qxUKLvasqKmUry3cN9KbTZLzYEgKKySVR8TCIVm3lgk" );
	...
	*/
	...
	/* Call this method to point future requests to your extended class */
	ts->change_requests_class<MyTwitterRequest>();
	...
	/* Note that you can do this with instances :

	ts->set_next_request( new MyTwitterRequest(ts) );

	// Returned request will be your "MyTwitterRequest"
	TwitterRequest* req = ts->stream_user_request();
